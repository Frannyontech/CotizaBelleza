...
export const productService = {
  ...
  // Obtener productos de DBS
  getDBSProducts: async (filters = {}) => {
    try {
      const params = new URLSearchParams();
      ...
      console.log('Making API request to:', `productos-dbs/?${params.toString()}`);
      const response = await api.get(`productos-dbs/?${params.toString()}`);
      console.log('DBS API response:', response.data);
      return response.data;
    } catch (error) {
      console.error('Error fetching DBS products:', error);
      console.error('Error details:', error.response?.data || error.message);
      throw error;
    }
  },

  // Buscar productos en DBS
  searchDBSProducts: async (searchTerm) => {
    try {
      const response = await api.get(`productos-dbs/?search=${encodeURIComponent(searchTerm)}`);
      return response.data;
    } catch (error) {
      console.error('Error searching DBS products:', error);
      throw error;
    }
  },
  ...
  // Obtener productos de Maicao
  getMaicaoProducts: async (filters = {}) => {
    try {
      const params = new URLSearchParams();
      ...
      console.log('Making API request to:', `productos-maicao/?${params.toString()}`);
      const response = await api.get(`productos-maicao/?${params.toString()}`);
      console.log('Maicao API response:', response.data);
      return response.data;
    } catch (error) {
      console.error('Error fetching Maicao products:', error);
      console.error('Error details:', error.response?.data || error.message);
      throw error;
    }
  },

  // Buscar productos en Maicao
  searchMaicaoProducts: async (searchTerm) => {
    try {
      const response = await api.get(`productos-maicao/?search=${encodeURIComponent(searchTerm)}`);
      return response.data;
    } catch (error) {
      console.error('Error searching Maicao products:', error);
      throw error;
    }
  },
